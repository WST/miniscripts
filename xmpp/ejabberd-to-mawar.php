...
while($u_res = mysql_fetch_assoc($u)) {
	$sql = 'INSERT INTO ' . DB_MAWAR . '.users (user_username, user_password) VALUES (\'' . mysql_real_escape_string($u_res['username'], $cnx) . '\', \'' . mysql_real_escape_string($u_res['password'], $cnx) . '\')';
	mysql_query($sql, $cnx);
	...
	while($r_res = mysql_fetch_assoc($r)) {
		$sql = 'INSERT INTO ' . DB_MAWAR . '.roster (user_id, contact_jid, contact_nick, contact_group, contact_subscription, contact_pending) VALUES (' . $id_user . ', \'' . mysql_real_escape_string($r_res['jid'], $cnx) . '\', \'' . mysql_real_escape_string($r_res['nick'], $cnx) . '\', \'' . mysql_real_escape_string($r_res['grp'], $cnx) . '\', \'' . $r_res['subscription'] . '\', \'' . $r_res['ask'] . '\')';
		mysql_query($sql, $cnx);
	}
	...
	if($v_res = mysql_fetch_assoc($v)) {
		$sql = 'INSERT INTO ' . DB_MAWAR . '.vcard (user_id, vcard_data) VALUES (' . $id_user . ', \'' . mysql_real_escape_string($v_res['vcard'], $cnx) . '\')';
		mysql_query($sql, $cnx);
	}
	mysql_free_result($v);
